...
public class BuildCraftCows extends BaseCowHandler {
    ...
    public List<CowsRegistryItem> registerCows() {
        ...
        oilCow = tryAddCow("OilCow", 300, "oil", 0x1F1A12, 0x3A352A, SpawnType.NORMAL,
            new String[] { "en_US:Oil Cow", "ja_JP:オイル牛" });
        allCows.add(oilCow);

        fuelCow = tryAddCow("FuelCow", 301, "fuel", 0xE5CC00, 0xFFF280, SpawnType.NORMAL,
            new String[] { "en_US:Fuel Cow", "ja_JP:燃料牛" });
        allCows.add(fuelCow);

        redplasmaCow = tryAddCow("RedPlasmaCow", 302, "redplasma", 0xCC0000, 0xFF6666, SpawnType.HELL,
